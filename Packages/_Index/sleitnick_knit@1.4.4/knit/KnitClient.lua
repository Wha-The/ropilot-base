...
KnitClient.Controllers = controllers
...
function KnitClient.AddControllersDeep(parent: Instance): {Controller}
	...
	for _,v in ipairs(parent:GetDescendants()) do
		if not v:IsA("ModuleScript") then continue end
		local controller; task.defer(function()
			controller = require(v)
		end)
		local timeout = false; task.delay(5, function() timeout = true end)
		while not controller and not timeout do task.wait() end
		if timeout then
			warn("Knit.AddControllersDeep timed out while loading " .. v:GetFullName())
			continue
		end
