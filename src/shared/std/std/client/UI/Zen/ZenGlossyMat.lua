...
local GlossyMat = std.client.UI.GlossyMat

local ZenGlossyMat do --[=[ [LUAUPP]: Luau with Classes; inherit: []  ]=]local _ZenGlossyMat_ = {};
    function _ZenGlossyMat_.__init__(self, path)
        self.Path = path
    end
    function _ZenGlossyMat_.Compose(self, frame, _, maid)
        self.GlossyInstance = GlossyMat(std.parsePath(self.Path)(frame))
        maid:GiveTask(self)
    end
    function _ZenGlossyMat_.OnOpen(self)
        self.GlossyInstance:enable()
    end
    function _ZenGlossyMat_.OnClose(self)
        self.GlossyInstance:disable()
    end
    function _ZenGlossyMat_.enable(self)
        self.GlossyInstance:enable()
    end
    function _ZenGlossyMat_.disable(self)
        self.GlossyInstance:disable()
    end
    function _ZenGlossyMat_.Destroy(self)
        self.GlossyInstance:Destroy()
    end
 --[[ [luau with classes] little bit of code to make the constructor work :) ]] ZenGlossyMat=setmetatable(_ZenGlossyMat_,{__call=function(classconstructor_reference,...)local self={}local memaddr=tostring(self)self=setmetatable(self,{__index=function(_,i)local v=_ZenGlossyMat_[i]if typeof(v)=="function"then return function(...)local args=table.pack(...)if not rawequal(args[1],self)then return v(self,table.unpack(args))end;return v(table.unpack(args))end;end;if rawequal(v,nil)then if _ZenGlossyMat_.__index__ then return _ZenGlossyMat_.__index__(self,i)end;end;return v;end,__tostring=function(self)return self.__repr__ and self.__repr__()or"<classinstance of "..tostring("ZenGlossyMat").." at "..memaddr..">"end,__call=function(self,...)assert(self.__call__,tostring(self).." is not callable!")return self.__call__(...)end,__mul=function(a,b)local other=nil;if rawequal(a,self)then other=b;else other=a;end;assert(self.__mul__,"Attempted to multiply with incompatible type: "..tostring(self))return self.__mul__(other)end,__add=function(a,b)local other=nil;if rawequal(a,self)then other=b;else other=a;end;assert(self.__add__,"Attempted to add with incompatible type: "..tostring(self))return self.__add__(other)end,__eq=function(self,...)if self.__eq__ then return self.__eq__(...)else return rawequal(self,...)end;end,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__CONSTRUCTOR_REFERENCE=ZenGlossyMat,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__INHERITCLASSCONSTRUCTORS={},})if self.__init__ then self.__init__(...)end;return self;end,__tostring=function()return"<classconstructor ZenGlossyMat>"end,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__ISCONSTRUCTOR=true,})end
