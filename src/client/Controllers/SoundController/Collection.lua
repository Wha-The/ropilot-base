...
local SoundCollection do --[=[ [LUAUPP]: Luau with Classes; inherit: []  ]=]local _SoundCollection_ = {};
    function _SoundCollection_.__init__(self, sounds)
        self.Sounds = sounds
        self.CurrentSound = nil
        self._onSoundEnded = nil
    end

    function _SoundCollection_.Play(self, soundController, backgroundState)
        ...
        if self._onSoundEnded then
            self._onSoundEnded:Disconnect()
        end

        -- Choose and create new sound
        local chosenSound = std.random.choice(self.Sounds)
        self.CurrentSound = soundController:CreateSoundFromTemplate(chosenSound)
        
        -- Set up requeuing
        self._onSoundEnded = self.CurrentSound.Ended:Connect(function()
            self:Play(soundController, backgroundState)
        end)
    ...
    function _SoundCollection_.Destroy(self)
        if self._onSoundEnded then
            self._onSoundEnded:Disconnect()
            self._onSoundEnded = nil
        end
 ...
 --[[ [luau with classes] little bit of code to make the constructor work :) ]] SoundCollection=setmetatable(_SoundCollection_,{__call=function(classconstructor_reference,...)local self={}local memaddr=tostring(self)self=setmetatable(self,{__index=function(_,i)local v=_SoundCollection_[i]if typeof(v)=="function"then return function(...)local args=table.pack(...)if not rawequal(args[1],self)then return v(self,table.unpack(args))end;return v(table.unpack(args))end;end;if rawequal(v,nil)then if _SoundCollection_.__index__ then return _SoundCollection_.__index__(self,i)end;end;return v;end,__tostring=function(self)return self.__repr__ and self.__repr__()or"<classinstance of "..tostring("SoundCollection").." at "..memaddr..">"end,__call=function(self,...)assert(self.__call__,tostring(self).." is not callable!")return self.__call__(...)end,__mul=function(a,b)local other=nil;if rawequal(a,self)then other=b;else other=a;end;assert(self.__mul__,"Attempted to multiply with incompatible type: "..tostring(self))return self.__mul__(other)end,__add=function(a,b)local other=nil;if rawequal(a,self)then other=b;else other=a;end;assert(self.__add__,"Attempted to add with incompatible type: "..tostring(self))return self.__add__(other)end,__eq=function(self,...)if self.__eq__ then return self.__eq__(...)else return rawequal(self,...)end;end,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__CONSTRUCTOR_REFERENCE=SoundCollection,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__INHERITCLASSCONSTRUCTORS={},})if self.__init__ then self.__init__(...)end;return self;end,__tostring=function()return"<classconstructor SoundCollection>"end,__LUAUWITHCLASSES_INTERNAL_DO_NOT_MODIFY__ISCONSTRUCTOR=true,})end
